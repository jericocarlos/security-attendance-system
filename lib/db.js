...
// attendance_freemeal (Server: EWBPO-MEAP)
export const freemealPool = mysql.createPool({
  ...
  password: process.env.DB_PASSWORD,
  // *********************************
  database: 'attendance_freemeal',
  port: 3306,
  waitForConnections: true,
  connectionLimit: 10,
});

// module.exports = { attendancePool, freemealPool };

// Execute SQL queries
export async function executeQuery({ query, values = [] }) {
  try {
    // Use mysql2's parameterized text protocol instead of server-side prepared
    // statements. The configured MySQL/MariaDB server rejects stmt execution
    // for valid query arguments (ER_WRONG_ARGUMENTS / errno 1210).
    const [results] = await attendancePool.query(query, values);
    return results;
  } catch (error) {
